...
# Helper: check if a Docker IMAGE is already downloaded
image_exists() {
    docker images --format '{{.Repository}}' 2>/dev/null | grep -qF "$1"
}

# Helper: check if a named container exists (running or stopped)
container_exists() {
    docker ps -a --format '{{.Names}}' 2>/dev/null | grep -qw "$1"
}

# Helper: ask user to reinstall — triggered if image OR container is found
prompt_reinstall() {
    local CONTAINER_NAME="$1"
    local IMAGE="$2"
    local DISPLAY_NAME="$3"
    local _img_found=false
    local _con_found=false

    image_exists     "${IMAGE}"          && _img_found=true
    container_exists "${CONTAINER_NAME}" && _con_found=true

    # Build a human-readable reason string
    if [[ "$_img_found" == true && "$_con_found" == true ]]; then
        warn "${DISPLAY_NAME} image is already downloaded and a container exists."
    elif [[ "$_img_found" == true ]]; then
        warn "${DISPLAY_NAME} image is already downloaded (no container yet)."
    else
        warn "${DISPLAY_NAME} container '${CONTAINER_NAME}' already exists (image may be cached)."
    fi

    echo ""
    echo -e "  Would you like to ${RED}${BOLD}reinstall${RESET} it?"
    echo -e "  ${YELLOW}(This will stop/remove the existing container & image, then re-download.)${RESET}"
    echo ""
    echo -n "  Reinstall ${DISPLAY_NAME}? [y/N]: "
    read -r REINSTALL_CHOICE </dev/tty
    echo ""

    if [[ "${REINSTALL_CHOICE,,}" == "y" || "${REINSTALL_CHOICE,,}" == "yes" ]]; then
        log "Stopping and removing existing '${CONTAINER_NAME}' container (if any)..."
        docker stop "${CONTAINER_NAME}" >/dev/null 2>&1 || true
        docker rm   "${CONTAINER_NAME}" >/dev/null 2>&1 || true
        log "Removing existing image '${IMAGE}'..."
        docker rmi  "${IMAGE}" >/dev/null 2>&1 || true
        log "Re-downloading ${DISPLAY_NAME}..."
        pull_image "${IMAGE}" "${DISPLAY_NAME}"
    else
        info "Skipping reinstall of ${DISPLAY_NAME}. Keeping existing setup."
    fi
}

# ── Juice Shop: check before install ──────────────────────────────────────────
if image_exists "bkimminich/juice-shop" || container_exists "juice-shop"; then
    prompt_reinstall "juice-shop" "bkimminich/juice-shop" "OWASP Juice Shop"
else
    pull_image "bkimminich/juice-shop" "OWASP Juice Shop"
fi

# ── WebGoat: check before install ─────────────────────────────────────────────
if image_exists "webgoat/webgoat" || container_exists "webgoat"; then
    prompt_reinstall "webgoat" "webgoat/webgoat" "OWASP WebGoat"
else
...
---------------------------------------------------------------
  ...
  1. Open a terminal and run one of the commands below.
  2. Wait for the READY message, then open the URL shown
     in your browser.
  3. When you're done, stop the container with the Stop command.
...
---------------------------------------------------------------
  WebGoat — First Time Setup
---------------------------------------------------------------

  WebGoat requires you to create a local account before you
  can access the lessons. Here's how:

  Step 1: Run StartWebgoat and wait for the READY message.

  Step 2: Open your browser and go to:
              http://localhost:8080/WebGoat

  Step 3: Click "Register new user" on the login page.

  Step 4: Enter any username and password you like.
          (This is a local account — no real email needed.)

  Step 5: Click "Sign up" — you'll be logged in automatically.

  Step 6: You'll see the lesson list on the left sidebar.
          Start with "Introduction" → "WebGoat" for a tour.

  NOTE: Your account and progress are stored ONLY inside the
        Docker container. If you run StopWebgoat and delete
        the container, your progress will be lost.
        To keep progress across sessions, always use
        StopWebgoat (not docker rm) to shut down.
...
# ── Section: WebGoat first-time setup ────────────────────────────────────────
echo -e "${BOLD}${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo -e "${BOLD}  🔑 WebGoat — First Time Setup${RESET}"
echo -e "${BOLD}${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo ""
echo -e "  ${BOLD}1.${RESET} Run ${GREEN}StartWebgoat${RESET} and wait for the READY message."
echo -e "  ${BOLD}2.${RESET} Open: ${CYAN}http://localhost:8080/WebGoat${RESET}"
echo -e "  ${BOLD}3.${RESET} Click ${BOLD}\"Register new user\"${RESET} on the login page."
echo -e "  ${BOLD}4.${RESET} Enter any username and password (no real email needed)."
echo -e "  ${BOLD}5.${RESET} Click ${BOLD}\"Sign up\"${RESET} — you will be logged in automatically."
echo -e "  ${BOLD}6.${RESET} Start with ${BOLD}Introduction → WebGoat${RESET} in the left sidebar."
echo ""
echo -e "  ${YELLOW}⚠  Note:${RESET} Progress is stored inside the container."
echo "     Always use ${GREEN}StopWebgoat${RESET} (not docker rm) to preserve it."
echo ""
...
# ── Section: Instructions saved notice ────────────────────────────────────────
echo -e "${BOLD}${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo -e "  ${BOLD}📄 These instructions have been saved to a file.${RESET}"
echo -e "     You can read them anytime by running:"
echo ""
echo -e "     ${GREEN}cat \"${INSTRUCTIONS_FILE}\"${RESET}"
if [[ -d "$DESKTOP_DIR" ]]; then
echo ""
echo -e "     A copy was also placed on your ${BOLD}Desktop${RESET}:"
echo -e "     ${GREEN}${DESKTOP_DIR}/HackToLive-Lab-Instructions.txt${RESET}"
fi
echo -e "${BOLD}${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
echo ""
